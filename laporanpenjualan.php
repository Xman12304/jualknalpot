...
<button type="submit" name="submit" class="w3-btn w3-red w3-center w3-section">Search</button>
 <a onclick="printContent('div1')" class="w3-btn w3-red w3-center w3-section">Print</a><br>
	...
	<!--table pelanggan-->
  <div id="div1">	
  <h3 class="w3-text-red"><b>Laporan Data Penjualan</b></h3>
	...
	<tr>
		<?php
			...
			$query=mysqli_query($conn,"SELECT p.kd_pelanggan,p.nama_pelanggan,
			SUM(dp.harga_satuan*dp.jml_beli)+ 300.000 AS total,
			dp.nama_barang,dp.tgl_pembelian  
			FROM detil_pembelian dp,pelanggan p 
			WHERE dp.kd_pelanggan=p.kd_pelanggan AND MONTH(tgl_pembelian)='$_POST[bulan]' 
			GROUP BY dp.kd_pelanggan");
			...
			$rupiah=number_format($data['total'],3);
			echo "
				<tr>
				<td>$no</td>
